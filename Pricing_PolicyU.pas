...
begin
  if cTypeGoods(intgrfldPricingGType.Value) in [tCarpet, tTrack] then
    ...
    begin
      ...
      case cTypeGoods(intgrfldPricingGType.Value) of
        tCarpet:
          Text := FormatCurr('# ##0.00гр', curr * 1000000 /
              intgrfldPricingGLength.Value / intgrfldPricingGWidth.Value);
        tTrack:
          Text := FormatCurr('# ##0.00гр', curr * 1000000 / intgrfldPricingGWidth.Value);
      end
    end;
...
begin
  case cTypeGoods(intgrfldPricingGType.Value) of
    tTrack: Text := FormatCurr('# ##0.00гр мп', crncyfldPricingDPriceCoeff.Value * 1000);
  else
    Text := FormatCurr('# ##0.00гр', crncyfldPricingDPriceCoeff.Value);
  end;
end;
